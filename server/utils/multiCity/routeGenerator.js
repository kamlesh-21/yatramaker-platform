...
//     try {
//         console.log(`   Calculating travel legs for ${destinations.length} destinations...`);
        
//         // Leg 0: User → First destination
//         console.log(`   Leg 0: ${userLocation.name} → ${destinations[0].name}`);
//         const leg0Options = calculateTravelCost(userLocation, destinations[0], travellers);
        
//         console.log(`   Leg 0 options: ${leg0Options?.length || 0}`);
...
//         if (!selectedLeg0) {
//             console.warn(`   ⚠️  Could not select mode for first leg`);
//             return null;
//         }
...
//         console.log(`   ✓ Leg 0: ${selectedLeg0.mode} - ₹${selectedLeg0.cost}`);
...
//             console.log(`   Leg ${i + 1}: ${fromName} → ${toName}`);
...
//             console.log(`   Leg ${i + 1} options: ${legOptions?.length || 0}`);
...
//             console.log(`   ✓ Leg ${i + 1}: ${selectedLeg.mode} - ₹${selectedLeg.cost}`);
//         }
...
//         console.log(`   Final leg: ${lastDestName} → ${userLocation.name}`);
        
//         const finalLegOptions = calculateTravelCost(lastDest, userLocation, travellers);
        
//         console.log(`   Final leg options: ${finalLegOptions?.length || 0}`);
...
//         console.log(`   ✓ Final leg: ${selectedFinalLeg.mode} - ₹${selectedFinalLeg.cost}`);
//         console.log(`   ✅ All ${legs.length} legs calculated successfully`);
...
//     console.log(`   Valid options: ${validOptions.length} (${validOptions.map(o => `${o.mode}:₹${o.cost}`).join(', ')})`);
    
//     if (goal === 'budget' || goal === 'cheapest') {
//         const cheapest = validOptions.reduce((min, opt) => 
//             opt.cost < min.cost ? opt : min
//         );
//         console.log(`   Selected cheapest: ${cheapest.mode} at ₹${cheapest.cost}`);
//         return cheapest;
...
//         });
//         console.log(`   Selected fastest: ${fastest.mode}`);
//         return fastest;
...
//         );
//         console.log(`   Selected comfortable: ${best.mode}`);
//         return best;
...
//         );
//         console.log(`   Selected experiential: ${best.mode}`);
//         return best;
//     }
...
//         opt.cost < min.cost ? opt : min
//     );
//     console.log(`   Selected default (cheapest): ${cheapest.mode} at ₹${cheapest.cost}`);
//     return cheapest;
// }
...
//     // Add overhead
//     const overhead = {
//         flight: 3,    // Airport checkin, boarding, etc.
...
//     return baseTime + (overhead[mode] || 0);
// }
...
async function generateRoute(cluster, userQuery) {
    try {
        ...
        console.log('🔍 DEBUG - Legs travel time:');
        legs.forEach((leg, i) => {
            console.log(`   Leg ${i}: ${leg.from} → ${leg.to}`);
            console.log(`      duration: ${leg.duration}`);
            console.log(`      estimatedTimeHours: ${leg.estimatedTimeHours}`);
        });

        const totalTravelTime = legs.reduce((sum, leg) => {
            // Use whichever exists
            const time = leg.duration || leg.estimatedTimeHours || 0;
            console.log(`      Adding ${time}h to total`);
            return sum + time;
        }, 0);

        console.log(`   ✅ Total travel time: ${totalTravelTime.toFixed(1)}h`);
...
async function calculateAllLegs(destinations, userLocation, travellers, optimizationGoal) {
    ...
    try {
        ...
        if (!selectedLeg0.duration && !selectedLeg0.estimatedTimeHours) {
            ...
            console.log(`   ⚠️ Added missing duration: ${selectedLeg0.duration}h`);
        }
        ...
        if (!selectedLeg.duration && !selectedLeg.estimatedTimeHours) {
            ...
            console.log(`   ⚠️ Added missing duration: ${selectedLeg.duration}h`);
        }
        ...
        if (!selectedFinalLeg.duration && !selectedFinalLeg.estimatedTimeHours) {
            ...
            console.log(`   ⚠️ Added missing duration: ${selectedFinalLeg.duration}h`);
        }
...
function estimateTravelTime(travelOption) {
    ...
    console.log(`   ⏱️ Estimated ${mode} time for ${distance}km: ${totalTime.toFixed(1)}h`);
